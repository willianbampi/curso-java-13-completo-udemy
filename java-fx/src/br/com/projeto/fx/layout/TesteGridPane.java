package br.com.projeto.fx.layout;

import javafx.scene.layout.ColumnConstraints;
import javafx.scene.layout.GridPane;
import javafx.scene.layout.RowConstraints;

public class TesteGridPane extends GridPane {
	
	public TesteGridPane() {
		Caixa c1 = new Caixa().comTexto("Texto 1");
		Caixa c2 = new Caixa().comTexto("Texto 2");
		Caixa c3 = new Caixa().comTexto("Texto 3");
		Caixa c4 = new Caixa().comTexto("Texto 4");
		Caixa c5 = new Caixa().comTexto("Texto 5");
		Caixa c6 = new Caixa().comTexto("Texto 6");
		
		setGridLinesVisible(true);
		
		getColumnConstraints().addAll(columnConstraints(), columnConstraints(), fixoColumnConstraints(), columnConstraints(), columnConstraints());
		getRowConstraints().addAll(rowConstraints(), rowConstraints(), rowConstraints(), fixoRowConstraints(), rowConstraints());
		
		setVgap(10);
		setHgap(10);
		
		add(c1, 0, 0, 2, 2);
		add(c2, 1, 1, 2, 2);
		add(c3, 4, 2, 1, 3);
		add(c4, 3, 1);
		add(c5, 0, 4, 2, 1);
		add(c6, 3, 3);
	}
	
	private ColumnConstraints columnConstraints() {
		ColumnConstraints cc = new ColumnConstraints();
		cc.setPercentWidth(20.0);
		cc.setFillWidth(true);
		return cc;
	}
	
	private RowConstraints rowConstraints() {
		RowConstraints rc = new RowConstraints();
		rc.setPercentHeight(20.0);
		rc.setFillHeight(true);
		return rc;
	}
	
	private ColumnConstraints fixoColumnConstraints() {
		ColumnConstraints cc = new ColumnConstraints();
		cc.setMinWidth(100);
		cc.setMaxWidth(100);
		cc.setFillWidth(true);
		return cc;
	}
	
	private RowConstraints fixoRowConstraints() {
		RowConstraints rc = new RowConstraints();
		rc.setMinHeight(100);
		rc.setMaxHeight(100);
		rc.setFillHeight(true);
		return rc;
	}

}
